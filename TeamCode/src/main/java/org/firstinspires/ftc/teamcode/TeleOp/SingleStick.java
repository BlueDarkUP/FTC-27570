...
public class SingleStick extends LinearOpMode {
    private static final double CLAW_INCREMENT = 0.4, DEBOUNCE_DELAY = 200, SLIDE_DOWN_POWER = -0.6, FRAME_HOLD_POSITION = 0, FRAME_INITIAL_POSITION = 0.7, CLAW_SHU_ROTATE_POSITION = 1.0, CLAW_SHU_INITIAL_POSITION = 0.0, SERVO_SPEED_MULTIPLIER = 0.5, FRAME_SERVO_SPEED = 1, DRIVE_STOP_THRESHOLD = 0.01;
    private static final int SLIDE_HIGH = 2580, SLIDE_MID = 1696, SLIDE_HANG = 250, SLIDE_HOME = 0, SLIDE_HIGH_START = 2000;

    private enum SlideState { IDLE, MOVING_TO_POSITION, MANUAL_DOWN }
    private SlideState slideState = SlideState.IDLE;

    private boolean isClawShuRotating = false, isFrameMoving = false, isAutoSlideDown = false, isClawOpen = false, isForwardClawOpen = false, isClawHengOpen = true, armForwardPosition = true, isFirstReset = true;
    ...
    private void controlServos() {
        if (gamepad1.dpad_up && debounce(lastForwardButtonPressTime)) {
            ...
            arm_forward.setPosition(0.35);  // Add this line
        ...
        if (gamepad1.cross && debounce(lastCrossButtonPressTime)) {
            ...
            arm_forward.setPosition(armForwardPosition ? 0.15 : 0.35);
        ...
        if (gamepad1.triangle && debounce(lastTriangleButtonPressTime)) {
            ...
            claw_heng.setPosition(isClawHengOpen ? 0.2 : 0.55);
    ...
    private void handleSlideMovement() {
        ...
        if (gamepad1.y) {
            ...
            if (gamepad1.left_bumper) {
                ...
                handleDpadLeft();  // 调用新方法处理 dpad_left
            } else if (gamepad1.dpad_right) {
                handleDpadRight(); // 调用新方法处理 dpad_right
            }
            ...
            if (isSlideBusy()) {
                if (isAutoSlideDown && Left_Hanging_Motor.getCurrentPosition() >= SLIDE_HIGH && !isFrameMoving) {
                    ...
                    telemetry.addData("滑轨状态", "等待 frame 移动完成...");
                    telemetry.update();
                } else {
                    Left_Hanging_Motor.setZeroPowerBehavior(DcMotor.ZeroPowerBehavior.BRAKE);
                    Right_Hanging_Motor.setZeroPowerBehavior(DcMotor.ZeroPowerBehavior.BRAKE);
